...
\begin{document}
    ...
    \begin{defn}
        An \ii{undirected graph} $\mc{G}$ is an ordered pair $(V(\mc{G}),E(\mc{G}))$, consisting of a set $V(\mc{G})$ of \ii{vertices}
    ...
    \begin{defn}
        A graph is \ii{simple} if it has neither different edges connecting the same pair of vertices nor loops, i.e. there is a unique edge joining two different vertices
