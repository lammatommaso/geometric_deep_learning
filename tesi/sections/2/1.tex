...
\begin{document}
    ...
    \begin{defn}
        Let $\mc{G}$ be a graph, we define the \ii{adjacency matrix} $A$ of $\mc{G}$ as follows,
        $A$ is a $n \times n$ matrix, where $n$ is the number of vertices, such that
        \[a_{ij} = 
        \begin{cases}
            1 & \text{if there exists an edge joining $i$ to $j$} \\
            0 & \text{otherwise} \\
        \end{cases}. \]        
    \end{defn}

    \begin{rem}
        We used "to" instead of "with" in the previous definition in order to consider directed graphs.
        In fact for a directed graph the adjacency matrix is not symmetric.
    \end{rem}

    \begin{defn}
        A graph is \ii{simple} if it has neither parallel edges nor loops, i.e. there is a unique edge joining two different vertices
        and no vertex is joined to itself by an edge. This implies $a_ii = 0$ in the adjacency matrix.
    \end{defn}
    ...
    We recall that $C_0$ and $C_1$ are the groups of $0$ and $1$-chains respectively. We therefore denote by $\ch{i}$
    the vertex $i$ as a group element of $C_0$, and we denote $\ch{i,j}$ the edge joinig $\ch{i}$ to $\ch{j}$.
